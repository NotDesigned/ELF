...
3. The selected file in
   [`experiments/campaigns/`](../experiments/campaigns/): run IDs, comparison
   block, resources, immutable image, storage, and retry budget.
...
First review the fully expanded campaign and validate the relevant code and
config. Then use one run at a time while establishing a new execution path:

```bash
CTL="python scripts/experimentctl.py CAMPAIGN.yml"
$CTL prepare --run RUN_ID
...
`prepare` and dry-run write controller-local metadata. `stage` changes remote
storage. Non-dry-run `submit` changes the scheduler. Both `stage` and `submit`
enforce their own scoped preflight; use the explicit `preflight` command when a
readiness report is needed before mutation.
